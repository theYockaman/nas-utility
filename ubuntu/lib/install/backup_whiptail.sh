...
MAIN_SCRIPT="$SCRIPT_DIR/backup.sh"
...
TARGET_BIN=$(whiptail --inputbox "Path to install the backup runner (target binary)" 10 60 "/usr/local/bin/rsync-backup.sh" 3>&1 1>&2 2>&3)
...
if ! whiptail --yesno "Install backup runner with:\n\nTarget: ${TARGET_BIN}\nCron time: ${CRON_TIME}\nUser crontab: ${INSTALL_USER:-no}\nDry-run: ${DRY_RUN:-no}\n\nContinue?" 16 70; then
    echo "Aborted by user."
    exit 1
fi

exec sudo bash "$MAIN_SCRIPT" --target "${TARGET_BIN}" --time "${CRON_TIME}" ${INSTALL_USER} ${DRY_RUN}
